...
const JoinButton = ({ loading, isSubmitted = false, disabled = false, onClick = () => { } }: JoinButtonProps) => {
    return (
        <button onClick={onClick} className={mergeCN('mx-auto my-10 h-fit w-fit font-semibold bg-black rounded-[5rem] text-gray-400', disabled && "shadow-[0px_0px_50px_10px_white] text-white", !loading && "px-4 py-3 border-[1px] border-gray-600")}>
            {loading ?
